...
bool btPrintCanRx = 0;
bool btEnabled = 1;                 // Ensure this is set to true for testing
bool btH_Enabled = 0;
...
// Buffer to store joystick inputs
int joystickInputs[6];
...
void setup() {
  ...
  // Initialize motor and servo buffers
  ...
  for (int ServoID = 0; ServoID < 6; ServoID++) {
    ServoPos[ServoID][0] = 0;
    ServoPos[ServoID][1] = 0;
    ServoPos[ServoID][2] = 0;
    ServoPos[ServoID][3] = 0;
  }
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    switch (cIncomingByte) {
      ...
      case 'h':
      case 'H':  // Heartbeat CAN with generic heartbeat
      {
        btH_Enabled ^= 1;
      ...
      {
        // Existing motor code (do not modify)
        btEnabled = 1;
        btH_Enabled = 0;
        btIsWEB = 0;
        cIncomingByte = Serial.read();
      ...
      case 'X':  // Handle joystick data from Raspberry Pi
      {
        // Read the 6 bytes of joystick input data
        ...
        // Print received joystick data
        Serial.print("Joystick Inputs: ");
        for (int i = 0; i < 6; i++) {
  ...
  if (xQueueReceive(CAN_cfg.rx_queue, &rx_frame, 3 * portTICK_PERIOD_MS) == pdTRUE) {
    ...
    if ((ubDeviceType == 0x02) && (ubAPIClass == 0x06) && (ubAPIIndex == 0x01))  // assuming there are 6 motor drivers addressed 1 to 6
    {
      ...
      fMotorSpeed[ubDeviceNumber - 1] = (float)((rx_frame.data.u8[3] << 24) | (rx_frame.data.u8[2] << 16) | (rx_frame.data.u8[1] << 8) | rx_frame.data.u8[0]);
      iMotorCurrent = (rx_frame.data.u8[6] << 8) | rx_frame.data.u8[7];
      fMotorCurrent[ubDeviceNumber - 1] = (iMotorCurrent / 4095.0f) * 80;
  ...
  if ((ul5mS % 50) == 0) // Send joystick inputs as a single CAN packet every 50ms
  {
    if (btEnabled) {
      tx_frame.FIR.B.FF = CAN_frame_ext;
      tx_frame.MsgID = 0x0C0C1801;  // CAN ID for joystick inputs
      tx_frame.FIR.B.DLC = 6;       // Data Length Code is 6
      ...
      // Print sent joystick data
      Serial.print("Sent Joystick Inputs over CAN: ");
      Serial.print(tx_frame.data.u8[0]); Serial.print(", ");
      Serial.print(tx_frame.data.u8[1]); Serial.print(", ");
      Serial.print(tx_frame.data.u8[2]); Serial.print(", ");
      Serial.print(tx_frame.data.u8[3]); Serial.print(", ");
      Serial.print(tx_frame.data.u8[4]); Serial.print(", ");
      Serial.println(tx_frame.data.u8[5]);
    }
  }
}
